...
function _isMealPlaceForRefs(place) {
  ...
  return (
    ...
    // 백화점·몰 내 다이닝/푸드홀 — Naver 카테고리가 '백화점/쇼핑'이어도 식사 목적지
    // (예: 신세계백화점 본점 더 리저브 = 프리미엄 푸드홀). 식사 슬롯에서 카드가
    // 텍스트로 떨어지지 않도록 허용한다.
    /더\s*리저브|리저브|푸드\s*홀|푸드\s*코트|푸드\s*마켓|식당가|다이닝|델리|그로서란트|food\s*hall|food\s*court/.test(blob) ||
    // 기타 식사 키워드
