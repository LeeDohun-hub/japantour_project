...
## AR 표본 확대 재판정 (n=48, 2026-06-25)

위 "표본 확대 후 재판정" 권고를 실행했다. 평가 파이프라인을 경량화해
(`--metrics answer_relevancy,context_recall`로 비싼 faithfulness/context_precision 생략)
n=48(6 카테고리 × 8)에서 AR을 재측정했다.

| 지표 | n=24(scope7) | **n=48** | 변화 |
| --- | ---: | ---: | ---: |
| Answer Relevancy | 0.6708 | **0.6941** | **+0.0233** |
| Context Recall | 0.9167 | 0.9583 | +0.0416 |
| retrieval_hit_rate | 0.9583 | 0.9792 | +0.0209 |
| retrieval_mrr | 0.8854 | 0.9323 | +0.0469 |
| route_accept_rate | 1.000 | 1.000 | ±0.000 |

**판정**: n=24가 분산으로 AR을 과소 보고했음이 확인됐다. 안정화된 실측 AR은 **0.694**로,
목표 0.80까지 격차는 0.129 → **0.106**으로 줄었다.

- 잔여 격차는 답변 품질 결함이 아니다 — faithfulness·precision·recall 모두 양호(0.86~0.96).
  광범위 질문("どのような場所?")의 RAGAS 역질문 불일치(측정 특성) + 잔여 노이즈가 지배적이다.
- 프롬프트 레버(원칙8)는 무효였고, 남은 레버인 "질문유형별 상세/간결 모드"는 측정 특성
  영역이라 ROI·faithfulness 리스크가 크다. **추가 프롬프트 튜닝보다 측정 안정화가 더 정직한
  결론**이며, AR 0.694는 양호 수준으로 본다.
- 결과 파일: `evaluation/reports/chat_corpus_ar_n48.json`

## 멀티턴 평가셋 — 보류 (저ROI, 2026-06-25)

검색 적중 미달의 잔여 원인인 지시어 후속질문(`このシャツ`, `この商品`, `この店`)을
멀티턴 평가로 다룰지 검토했다. 코퍼스 전체에서 대상은 **4 source record·5건**뿐이고,
핵심 케이스가 깔끔히 해소되지 않는다:

- `J_FOOD_000070_q1`("**この店**の名前は?") — q1이라 앞 턴이 없다. 선행사가 대화가 아니라
  소스 문서의 주제라 형제 질문으로 해소 불가(합성 컨텍스트 턴이 필요).
- `J_NAT_000010_q2`("なぜ**アバイ村**はその名前に?") — 엔티티가 질문에 이미 포함 → 단일턴 검색 가능.
- `J_SHOP_000001_q4/q6`("**このシャツ**") — q1은 브랜드(8 seconds)만 명명, 셔츠 자체는 아님.

제대로 하려면 합성 컨텍스트 턴 생성 + history-aware 쿼리 재작성(프로덕션 기능)까지 필요한데
대상이 5건이라 투자 대비 효과가 작다. **단일턴 평가의 알려진 데이터 한계로 기록하고 보류**한다.
(향후 대화형 UX에 멀티턴 검색을 도입할 때 함께 다루는 것이 적절하다.)
